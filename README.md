...
Bash
git clone https://github.com/gusthescripter/home_lab-stack_1.git
cd home_lab-stack_1
2. Configure Environment Variables
Create a local .env file in the root directory (this file is excluded from source control via .gitignore). You can use .env.example as a template:

Plaintext
GRAFANA_ADMIN_USER=your_secure_username
GRAFANA_ADMIN_PASSWORD=your_secure_password
PI_IP_ADDRESS=your_raspberry_pi_ip
3. Spin Up the Stack
Bash
docker compose up -d
4. Access Ports
Once initialized, access the resources via standard HTTP routing:

Target Web Application: http://<YOUR_PI_IP>/

Telemetry Dashboard: http://<YOUR_PI_IP>/grafana

🛑 Tear Down
To cleanly destroy the stack and wipe any temporary database volumes hanging around for a true factory reset, run:

Bash
docker compose down -v
Maintained as an independent security research project.
